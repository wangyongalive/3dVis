define([
...
], function (t, e, a, n, i, r, o, s, m, u, c) {
  'use strict'
  const l = new n.Cartesian3(),
    ...
    C = new n.Cartesian3(),
    p = new n.Cartesian3(),
    y = new n.Cartesian3(1, 1, 1),
    _ = Math.cos,
    h = Math.sin
  function x(t) {
    t = r.defaultValue(t, r.defaultValue.EMPTY_OBJECT)
    const e = r.defaultValue(t.radii, y),
      a = r.defaultValue(t.innerRadii, e),
      o = r.defaultValue(t.minimumClock, 0),
      s = r.defaultValue(t.maximumClock, i.CesiumMath.TWO_PI),
      m = r.defaultValue(t.minimumCone, 0),
      u = r.defaultValue(t.maximumCone, i.CesiumMath.PI),
      l = Math.round(r.defaultValue(t.stackPartitions, 64)),
  ...
  ;(x.unpack = function (t, e, a) {
    ...
    const i = n.Cartesian3.unpack(t, e, A)
    e += n.Cartesian3.packedLength
    const o = n.Cartesian3.unpack(t, e, b)
    e += n.Cartesian3.packedLength
    const s = c.VertexFormat.unpack(t, e, k)
    ...
    (x.createGeometry = function (t) {
      ...
      let w,
        F,
        P = t._slicePartitions + 1,
        g = t._stackPartitions + 1
      ;(P = Math.round((P * Math.abs(x - y)) / i.CesiumMath.TWO_PI)),
        (g = Math.round((g * Math.abs(b - A)) / i.CesiumMath.PI)),
        P < 2 && (P = 2),
        g < 2 && (g = 2)
      let v = 0
      const V = [A],
        M = [y]
      for (w = 0; w < g; w++) V.push(A + (w * (b - A)) / (g - 1))
      for (V.push(b), F = 0; F < P; F++) M.push(y + (F * (x - y)) / (P - 1))
      M.push(x)
      const T = V.length,
        D = M.length
      let G = 0,
        L = 1
      const O = c.x !== o.x || c.y !== o.y || c.z !== o.z
      let I = !1,
        E = !1,
        z = !1
      O &&
      ...
      const K = new Array(D),
        Q = new Array(D)
      for (F = 0; F < D; F++) (Q[F] = _(M[F])), (K[F] = h(M[F]))
      for (w = 0; w < T; w++)
        for (F = 0; F < D; F++)
          (R[v++] = o.x * q[w] * Q[F]),
            (R[v++] = o.y * q[w] * K[F]),
        ...
        for (w = 0; w < T; w++)
          for (F = 0; F < D; F++)
            (R[v++] = c.x * q[w] * Q[F]),
              ...
              (R[v++] = c.z * H[w]),
              (U[nt] = !0),
              w > 0 && w !== T - 1 && 0 !== F && F !== D - 1 && (S[nt] = !0),
              nt++
      for (v = 0, w = 1; w < T - 2; w++)
        for ($ = w * D, tt = (w + 1) * D, F = 1; F < D - 2; F++)
          (Y[v++] = tt + F),
            (Y[v++] = tt + F + 1),
            (Y[v++] = $ + F + 1),
            (Y[v++] = tt + F),
      ...
      if (O) {
        const t = T * D
        for (w = 1; w < T - 2; w++)
          for ($ = t + w * D, tt = t + (w + 1) * D, F = 1; F < D - 2; F++)
            (Y[v++] = tt + F),
              (Y[v++] = $ + F),
              (Y[v++] = $ + F + 1),
              (Y[v++] = tt + F),
      ...
      const it = new m.GeometryAttributes()
      k.position &&
        (it.position = new s.GeometryAttribute({
      ...
      const ut = N / 2
      let ct
      const lt = n.Ellipsoid.fromCartesian3(o),
        ft = n.Ellipsoid.fromCartesian3(c)
      if (k.st || k.normal || k.tangent || k.bitangent) {
        for (w = 0; w < N; w++) {
          ct = U[w] ? ft : lt
          const t = n.Cartesian3.fromArray(R, 3 * w, l),
            e = ct.geodeticSurfaceNormal(t, f)
          if ((S[w] && n.Cartesian3.negate(e, e), k.st)) {
            const t = n.Cartesian2.negate(e, p)
            ;(j[rt++] = Math.atan2(t.y, t.x) / i.CesiumMath.TWO_PI + 0.5),
          ...
          if (
            ...
            if (
              ...
              const a = n.Cartesian3.cross(e, t, C)
        ...
        k.st &&
          (it.st = new s.GeometryAttribute({
            componentDatatype: i.ComponentDatatype.FLOAT,
            componentsPerAttribute: 2,
            values: j
          })),
          k.normal &&
            (it.normal = new s.GeometryAttribute({
              componentDatatype: i.ComponentDatatype.FLOAT,
              componentsPerAttribute: 3,
              values: J
            })),
          k.tangent &&
            (it.tangent = new s.GeometryAttribute({
              componentDatatype: i.ComponentDatatype.FLOAT,
              componentsPerAttribute: 3,
              values: X
            })),
          k.bitangent &&
            (it.bitangent = new s.GeometryAttribute({
              componentDatatype: i.ComponentDatatype.FLOAT,
              componentsPerAttribute: 3,
              values: Z
            }))
      ...
      if (r.defined(t._offsetAttribute)) {
        const a = R.length,
          ...
          r = t._offsetAttribute === e.GeometryOffsetAttribute.NONE ? 0 : 1
        e.arrayFill(n, r),
          (it.applyOffset = new s.GeometryAttribute({
      ...
      return new s.Geometry({
        ...
        boundingSphere: a.BoundingSphere.fromEllipsoid(lt),
