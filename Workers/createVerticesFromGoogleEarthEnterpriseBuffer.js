define([
  ...
  function x(t, e, n) {
    ...
    const o = t.length
    for (let i = 0; i < o; ++i)
      if (n.equalsEpsilon(t[i], e, a.CesiumMath.EPSILON12)) return i
    return -1
  }
        ...
        for (Z = 0, X = 0; X < 4; ++X) {
          ...
          for (let t = 0; t < l; ++t) {
            ...
            ;(m[t] = gt),
              Math.abs(s - _) < r
                ...
                : Math.abs(s - v) < r
                  ? dt.push({
                      index: gt,
                      cartographic: n.Cartographic.clone(N)
                    })
                  : Math.abs(l - W) < h
                    ? ht.push({
                        index: gt,
            ...
            ;(E = a.CesiumMath.clamp(E, 0, 1)),
              (ot[gt] = new n.Cartesian2(I, E)),
              ++gt
          }
          const p = 3 * g
          for (let t = 0; t < p; ++t, ++mt)
            (ct[mt] = m[G.getUint16(Z, !0)]), (Z += E)
