define([
  ...
  const N = new e.Cartographic(),
    ...
    p = new e.Matrix4(),
    y = new e.Matrix2()
  ;(I._textureCoordinateRotationPoints = function (t, n, a, r) {
    let o
    const s = e.Rectangle.center(r, N),
      u = e.Cartographic.toCartesian(s, a, T),
      I = i.Transforms.eastNorthUpToFixedFrame(u, a, c),
      b = e.Matrix4.inverse(I, c),
      C = L,
      h = l
    ;(h[0].longitude = r.west),
      (h[0].latitude = r.south),
      (h[1].longitude = r.west),
      (h[1].latitude = r.north),
      (h[2].longitude = r.east),
      (h[2].latitude = r.south)
    let A = f
    for (o = 0; o < 3; o++)
      e.Cartographic.toCartesian(h[o], a, A),
        (A = e.Matrix4.multiplyByPointAsVector(b, A, A)),
        (C[o].x = A.x),
        (C[o].y = A.y)
    const d = i.Quaternion.fromAxisAngle(e.Cartesian3.UNIT_Z, -n, m),
      x = e.Matrix3.fromQuaternion(d, p),
      S = t.length
    ...
    for (o = 0; o < S; o++)
      (A = e.Matrix4.multiplyByPointAsVector(b, t[o], A)),
        (A = e.Matrix3.multiplyByVector(x, A, A)),
        (P = Math.min(P, A.x)),
        (G = Math.min(G, A.y)),
        (R = Math.max(R, A.x)),
        (_ = Math.max(_, A.y))
    const O = e.Matrix2.fromRotation(n, y),
      g = E
    ;(g[0].x = P),
      (g[0].y = G),
      (g[1].x = P),
